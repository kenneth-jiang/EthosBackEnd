class Api::V1::WolframController < ApplicationController
  ...
  def search
    byebug
    Search.create(search_term: params['searches'], user_id: my_user.id)
    resp = RestClient.get('http://api.wolframalpha.com/v2/query?appid='+ENV['WOLFRAM_KEY']+"&input="+params['searches']+"&format=image&output=json")
    resp_json = JSON.parse(resp)
    render json: resp_json
  end
